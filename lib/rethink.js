...
// LIST DATABASES: lists dbs from local rethinkdb server
module.exports.listDatabases = async(options = {}) => {
  let settings = setOpts(options);
  let dbs = [];

  try {
    const connection = await connectDb(settings);

    dbs = await r.dbList().run(connection);
    connection.close();
  } catch (err) {
    console.log(err);
  }
